...
Integration tests use a `data_hub_test` Postgres database and spin up a real Next.js production server. See [Testing](testing.md) for the full per-package breakdown and [CI and deployment](ci-and-deployment.md) for how these run in GitHub Actions.
