...
- **Google OAuth with database sessions.** Better Auth signs people in with Google Workspace and stores sessions in the `session` table, with a short-lived signed cookie cache so most page renders avoid a DB round-trip. The Drizzle adapter also writes `user` and `account` rows, so members and their linked Google identities stay queryable. Google is the only gate on who gets in, since Data Hub has no email allowlist or domain check of its own; `ADMIN_EMAILS` is a one-way promotion applied on sign-in, not an entry check. The `is_admin` flag is exposed on the session for cheap UI gating, but every privileged mutation re-reads it from the database, so a demotion takes effect on the next request instead of the next sign-in. Non-production builds also enable email/password for the "Sign in (dev)" form, described in [Local development](local-development.md#sign-in-dev-only).
