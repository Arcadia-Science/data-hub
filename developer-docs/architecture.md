...
- **Google OAuth with JWT sessions.** Auth.js (`next-auth` v5) signs people in with Google and keeps no session rows: a session is a signed JWT in a cookie. The Drizzle adapter still writes `user` and `account` rows, so members and their linked Google identities stay queryable. Google is the only gate on who gets in, since Data Hub has no email allowlist or domain check of its own; `ADMIN_EMAILS` is a one-way promotion applied on sign-in, not an entry check. The `is_admin` flag is cached on the JWT for cheap UI gating, but every privileged mutation re-reads it from the database, so a demotion takes effect on the next request instead of the next sign-in. Non-production builds also register an email-only credentials provider, described in [Local development](local-development.md#sign-in-dev-only).
