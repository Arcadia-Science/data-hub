...
- **Usage analytics.** Production sends Vercel Web Analytics custom events from `web/lib/analytics/track.ts` (`VERCEL_ENV === "production"` only; local development prints them). Each event carries the Data Hub user id and nothing that identifies a run, file, or search. MCP tools, resources, and prompts are wrapped once in `web/lib/mcp/analytics.ts`; `initialize` is `mcp_connect` with the client name and version. Web actions (claim, comment, download, search, reprocess, delete/restore, tokens, admin role) are recorded in the specific `/api/v1` route with `surface` `web` or `api`. A signed-in full page load records `web_visit`; a new session records `sign_in`. Read them in the Vercel Analytics Events panel, or group by `eventData/user_id` with the [Web Analytics API](https://vercel.com/docs/analytics/web-analytics-api). Automatic page views from `<Analytics />` stay anonymous and are separate from these events.
