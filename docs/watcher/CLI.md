...
3. **Upload queue polling.** On each heartbeat interval, the watcher polls `GET /api/v1/watchers/{watcher_id}/upload-queue` for runs that a user has selected for upload via the web UI (runs where `upload_requested_at` is set). For each queued run, the watcher uploads all associated files to S3, then notifies the API via `PATCH /api/v1/instruments/{instrument_id}/runs/{run_id}` with the S3 file records. Only runs that have already been reported (step 2) can appear in the upload queue.
