...
| `notion.api.create_page_in_database(...)` with properties and blocks | `POST /api/instruments/:instrumentId/runs` with structured JSON. |
...
| `notion.api.update_page_properties(...)` for analysis status | `PATCH /api/instruments/:instrumentId/runs/:runId` or the analyses sub-resource. |
...
2. Refactor one workflow at a time: modify `generate_report` to construct an API payload and call `POST /api/instruments/:instrumentId/runs` instead of the Notion API.
