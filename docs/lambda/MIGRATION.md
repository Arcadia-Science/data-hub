...
| `notion.api.create_page_in_database(...)` with properties and blocks | `POST /api/instrument-runs` with structured JSON. |
...
| `notion.api.update_page_properties(...)` for analysis status | `PATCH /api/instrument-runs/:id` or the analyses sub-resource. |
...
- Reserved concurrency = 1 — still needed to avoid race conditions on upsert.

## Migration Path

1. Add a Data Hub API client module to the Lambda codebase (similar to the watcher's `api_client.py`).
2. Refactor one workflow at a time: modify `generate_report` to construct an API payload and call `POST /api/instrument-runs` instead of the Notion API.
