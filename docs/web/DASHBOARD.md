...
**Content:**
- Summary cards per instrument: instrument name, count of runs, last run date, watcher status (online / offline / no watcher). For instruments with manual-mode watchers, show the count of files awaiting upload (files with `status` of `detected` or `upload_requested`) as a badge.
