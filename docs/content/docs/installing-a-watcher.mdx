...
description: Set up the Data Hub watcher on a lab instrument PC to monitor a directory, upload new files, and report run data to the web app.
---

This guide is for lab operators setting up the Data Hub watcher on an instrument PC. The watcher monitors a directory for new files, uploads them to cloud storage, and reports run data to the Data Hub web app.
...
- **A personal access token** — ask your Data Hub admin to create one for you, or create one yourself at **Settings > Access Tokens** in the web app (see [Managing tokens](/docs/managing-tokens)).
...
8. **Upload mode**:
   - **auto** — files are uploaded immediately after detection.
...
data-hub-watcher service reinstall # Stop, uninstall, install, and start
```
...
The watcher can upgrade itself in place, either on demand via `data-hub-watcher self-update` or — when running as a Windows service — automatically on an hourly background tick. The full release flow, mandatory-update behavior, and troubleshooting all live in [Upgrading the watcher](/docs/upgrading-the-watcher).
...
After a successful upgrade you must restart the watcher (or the Windows service) for the new code to take effect — `self-update` does not restart the running process.
...
Each instrument can have at most one active watcher at a time. If `init` fails with this error, an earlier install (often on a different PC, or before a reimage) is still registered against the instrument. The CLI prints the existing watcher's id; deregister it from the **Watchers** page in the web app before re-running `init`.

Deregistration is a soft-delete: heartbeats, events, and runs reported by the old watcher remain visible under **Watchers > Deregistered** for auditing.
...
- That the correct environment is set in the config (`staging` or `production`).
...
Files are 10 MB each with 5 backups kept (`watcher.log.1` through `watcher.log.5`). You can also pass `--verbose` for debug-level console output on the CLI:

```sh
data-hub-watcher --verbose watch
```

Running `data-hub-watcher watch` interactively while the Windows service is also running is not supported — both processes would race on the same rotating log file. Stop the service first if you need to run the CLI for debugging:
