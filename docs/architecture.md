...
- **Lambda-built run archives.** The "Download all" actions on a run delegate to the Lambda, which streams files from the raw bucket directly into a zip in a separate `arcadia-data-hub-archives-{env}` bucket via S3 multipart upload. The web app then 302s the browser to a short-lived presigned URL on that archive — bytes never traverse Vercel, eliminating Fast Origin Transfer for run downloads. Builds are cached at `runs/{instrument_id}/{run_id}/{fingerprint}.zip` and expire after 7 days. Builds smaller than `SYNC_ARCHIVE_THRESHOLD_GB` (default 25 GB) are awaited inside the request; larger builds skip the inline await up front, are tracked via the `archive_jobs` table, and the UI polls the original `/download-archive` URL (which HEADs S3 on each call) until the artifact appears — making the S3 object, not the row's `status`, the source of truth for "ready". See [Run archives](run-archives.md) for the full flow and runbook.
