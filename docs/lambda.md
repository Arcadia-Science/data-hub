...
4. On success, the Lambda returns `{ archive_bucket, archive_key, size_bytes }`. If `job_id` was supplied, it also PATCHes `/api/v1/archive-jobs/:job_id` with the same fields and `status: "ready"`; on failure it PATCHes `status: "failed"` with `error_message`. The PATCH callback authenticates with `Authorization: Bearer <LAMBDA_INVOKE_TOKEN>` (the same shared secret the inbound Function URL uses) — *not* `DATA_HUB_API_KEY`. Allowing a regular user PAT here would let any signed-in user redirect another user's download by writing a different `archive_bucket`/`archive_key`, so the route gates writes on a server-only secret. The PATCH primarily serves to record terminal state for diagnostics and to surface `failed` quickly — the UI's polling target is the `/download-archive` route itself (whose first action is an S3 HEAD against the canonical archive key), so a finished build is downloadable the moment the multipart upload completes regardless of whether this PATCH lands.
