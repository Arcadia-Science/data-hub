...
| `BETTER_AUTH_URL`           | Yes      | Public origin / OAuth issuer base (issuer `{origin}/api/auth`) |
...
| `MCP_ALLOW_PAT_AUTH`        | No       | Dev/CI only: MCP accepts PATs; hard-disabled on Vercel production and self-hosted production (non-loopback `BETTER_AUTH_URL`) |
